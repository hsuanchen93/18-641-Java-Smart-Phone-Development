...
public class Automobile implements Serializable {
	...
	/* PrintOptions - return String of all OptionSets along with Options */
	public String printOptions() {
		String result = "";
		for(int i=0; i<optset.size(); i++) {
			int num = optset.get(i).getOption().size();
			result += optset.get(i).getName()+":";
			for(int j=0; j<num; j++) {
				double price = optset.get(i).getOption(j).getPrice();
				result += optset.get(i).getOption(j).getName()+"($"+Double.toString(price)+")";
			}
			result+="\n";
		}
		return result;
